...
NOM = "Lecomte" # A REMPLIR
PRENOM = "Benjamin" # A REMPLIR
GROUPE = "4-1" # A REMPLIR
...
def evaluer_cnf(formule,list_var):
    ...
    Renvoie : None ou booléen
    '''
    l1 = copy.deepcopy(formule)
    for i in range(len(l1)):
        l1[i] = evaluer_clause(l1[i], list_var)
...
def determine_valuations(list_var):
    ...
    '''
    valuations = copy.deepcopy([list_var])
    ...
    for i in range(len(list_var)):
        if list_var[i] == None:
            ...
            for clause in valuations:
                true_valuation = clause.copy()
                false_valuation = clause.copy()

                true_valuation[i] = True
                false_valuation[i] = False

                new_listVar.append(true_valuation)
                new_listVar.append(false_valuation)
            
            valuations = new_listVar
...
def resol_sat_force_brute(formule,list_var):
    ...
    '''
    all_valuation = determine_valuations(list_var)
    for valuation in all_valuation:
...
def retablir_for(formule_init,list_chgmts):
    ...
    for i in range(len(list_chgmts)):
        if list_chgmts[i][1] == True:
            formule = enlever_litt_for(formule, list_chgmts[i][0]+1)
        elif list_chgmts[i][1] == False:
            formule = enlever_litt_for(formule, -list_chgmts[i][0]-1)
    return formule
...
def progress_simpl_for_dpll(formule,list_var,list_chgmts,list_sans_retour):
    ...
    for i in range(len(formule)):
        if len(formule[i]) <= 1:
            list_sans_retour.append(abs(formule[i][0])-1)
            if formule[i][0] < 0:
                list_var[abs(formule[i][0])-1] = False
                list_chgmts.append([abs(formule[i][0])-1, False])
                formule = retablir_for(formule, list_chgmts)
                print(formule, list_var, list_chgmts, list_sans_retour)
                return formule, list_var, list_chgmts, list_sans_retour
            else:
                list_var[abs(formule[i][0])-1] = True
                list_chgmts.append([abs(formule[i][0])-1, True])
                formule = retablir_for(formule, list_chgmts)
                print(formule, list_var, list_chgmts, list_sans_retour)
                return formule, list_var, list_chgmts, list_sans_retour
