...
'''
    ...
    for i in range(len(copy_form)):
        copy_form[i] = evaluer_clause(copy_form[i], list_var)

    if False in copy_form:
        return False
    elif None in copy_form:
        return None
    else:
        return True
    ...
    for i in range(len(list_var)):
        if list_var[i] == None:
            ...
            for valuation in valuations:
                true_valuation = valuation.copy()
                false_valuation = valuation.copy()

                true_valuation[i] = True

                false_valuation[i] = False

                new_valuations.append(true_valuation)
                new_valuations.append(false_valuation)
...
def retablir_for(formule_init,list_chgmts):
    ...
    for i in range(len(list_chgmts)):
        if list_chgmts[i][1] == True:
            result = enlever_litt_for(result, (list_chgmts[i][0])+1)
...
def retour_simpl_for(formule_init,list_var,list_chgmts):
    ...
    l2 : nouvelle list_chgmts 
    '''
    form = [clause[:] for clause in formule_init]
    l1 = list_var[:]
    l2 = list_chgmts[:]
    ...
    for sous_list_chgmts in reversed(l2):
        ...
        if value == True:
            sous_list_chgmts[1] = False
            l1[variable_index] = False
            form = retablir_for(form, l2)
            return form, l1, l2

        elif value == False:
            sous_list_chgmts[1] = None

            if l2[(l2.index(sous_list_chgmts)) - 1][1] == True:
                l2[(l2.index(sous_list_chgmts)) - 1][1] = False
                l1[l2[(l2.index(sous_list_chgmts)) - 1][0]] = False

            elif l2[(l2.index(sous_list_chgmts)) - 1][1] == False:
...
def resol_parcours_arbre_simpl_for(formule_init,formule,list_var,list_chgmts):#la même distinction peut être faite entre formule et formule_init
    ...
    formule_copy = [clause[:] for clause in formule]
    list_var_copy = list_var[:]
    list_chgmts_copy = list_chgmts[:]

    variable_index, value = list_chgmts_copy.pop()
    list_var_copy[variable_index] = value

    result, _ = eval_formule_simpl_for(formule_copy, list_var_copy)
